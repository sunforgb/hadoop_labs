...
class HDFSClient(cmd.Cmd):
    ...
    def do_mkdir(self, remote_path: str) -> bool:
        ...
        response = requests.put(urljoin(self.url, remote_path),
    ...
    def do_put(self, data: str, path: str) -> bool:
        ...
        response = requests.put(urljoin(self.url, path),
                                params=request_param,
    ...
    def do_append(self, file_path: str, path: str):
        ...
        response = requests.post(urljoin(self.url, path),
                                params=request_param,
                                allow_redirects=False)
        load_headers = {
            "content-type": "application/octet-stream"
    ...
    def do_delete(self, path: str):
        ...
        response = requests.delete(urljoin(self.url, path),
                                params=request_param)
        if response.status_code == 200:
            return response.json()["boolean"]
        else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="webhdfs client shell",
